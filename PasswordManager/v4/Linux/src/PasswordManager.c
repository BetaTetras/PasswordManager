...
int mainMenu(FILE *fichier,String MDP){
    String Input = (String)calloc(500,sizeof(char));
...
void ConsultationMDP(FILE *fichier){
    ...
    while(1){
        ...
        case 1:
            ...
            fgets(NomApplication,500,stdin);
...
void AjoutCompte(FILE *fichier){
	...
	for(int i=1;i<getNbElementListe(ListeApp);i++){
		if(strcmp(ListeApp[i],NomApp) == 0 && strcmp(ListeId[i],IdApp) == 0 ){
			...
			free(NomApp);free(IdApp);
			free(MDP);free(FinaleLigne);
			free(ListeApp);free(ListeId);
			pause();
			return;
		}
	}
...
int SetUp(){
    ...
    String MDP1 = (String)calloc(500,sizeof(char));
    String MDP2 = (String)calloc(500,sizeof(char));
