...
void DisplaySpecifiqueListe(FILE *fichier,String Element,int Colonne){
    String* ListeColonne = (String*)calloc(1000,sizeof(String));
        ListeColonne = getListeColonne(fichier,Colonne);

    for(int i=0;i<getNbElementListe(ListeColonne);i++){
        if(strcmp(Element,ListeColonne[i]) == 0){
            printf("%s : %s\n",getLigneXcolonneY(fichier,0,Colonne),getLigneXcolonneY(fichier,i,Colonne));
            for(int j=0;j<getNbColonne(fichier);j++){
...
void DisplaySpecifiqueTable(FILE *fichier,String Element,int Colonne){
    ...
    if(getNbElementListe(ListeLigne) == 0){
        Color_printf_RED("\n# Erreur...\n# Aucune application nomme '");
        Color_printf_RED(Element);
        Color_printf_RED("' trouvee ...\n\n");
        return;
    }
        ...
        if(BoolCouleur == 0){
            for(int i=1;i<getNbLigne(fichier);i++){
                if(strcmp(StringBuffer,getLigneXcolonneY(fichier,i,Colonne)) == 0){
                    for(int j=0;j<NbColonne;j++){
                    Color_printf_GRAY("|  ");
                    Color_printf_BLUE(getLigneXcolonneY(fichier,i,j));
                    printf(" ");
            ...
            for(int i=1;i<getNbLigne(fichier);i++){
                if(strcmp(StringBuffer,getLigneXcolonneY(fichier,i,Colonne)) == 0){
                    for(int j=0;j<NbColonne;j++){
                    Color_printf_GRAY("|  ");
                    Color_printf_YELLOW(getLigneXcolonneY(fichier,i,j));
                    printf(" ");
...
void DisplayOrdonnedListe(FILE *fichier,int colonne){
    ...
    for(int i=1;i<getNbLigne(fichier);i++){
        ...
        if(BoolFound == 0){
            ...
            if(BoolCouleur == 0){
                Color_printf_BLUE(getLigneXcolonneY(fichier,0,colonne));
                printf(" : ");
                Color_printf_BLUE(Buffer);
                printf("\n");
                    ...
                    if(strcmp(Buffer,getLigneXcolonneY(fichier,j,colonne)) == 0){
                        for(int k=1;k<getNbColonne(fichier);k++){
                            if(strcmp(getLigneXcolonneY(fichier,0,colonne),getLigneXcolonneY(fichier,0,k)) != 0){
                                printf("    ");
                                Color_printf_BLUE(getLigneXcolonneY(fichier,0,k));
                                printf(" : ");
                                Color_printf_BLUE(getLigneXcolonneY(fichier,j,k));
                                printf("\n");
                            }
                        }
                        printf("\n");
                ...
                Color_printf_YELLOW(getLigneXcolonneY(fichier,0,colonne));
                printf(" : ");
                Color_printf_YELLOW(Buffer);
                printf("\n");
                for(int j=1;j<getNbLigne(fichier);j++){
                    if(strcmp(Buffer,getLigneXcolonneY(fichier,j,colonne)) == 0){
                        for(int k=1;k<getNbColonne(fichier);k++){
                            if(strcmp(getLigneXcolonneY(fichier,0,colonne),getLigneXcolonneY(fichier,0,k)) != 0){
                                printf("    ");
                                Color_printf_YELLOW(getLigneXcolonneY(fichier,0,k));
                                printf(" : ");
                                Color_printf_YELLOW(getLigneXcolonneY(fichier,j,k));
                                printf("\n");
                            }
                        }
                        printf("\n");
