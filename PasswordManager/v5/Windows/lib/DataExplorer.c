...
/* =-=-=-=-=-= TOOLS =-=-=-=-=-= */
// Outil de navigation dans le fichier 
...
int getNbColonne(FILE *fichier){
    ...
    String Ligne = (String)calloc(1000,sizeof(char));    // Ligne buffer
    fgets(Ligne,1000,fichier);   // Récupère la premier ligne du fichier 
    int nbPipe = 0; // Variable compteur de pipe ("|")
    for(size_t i=0;i<strlen(Ligne);i++){   // de 0 aux nombre de caractère de la premier ligne du fichier faire ...
...
int getNbLigne(FILE *fichier){
    ...
    while(fgets(Ligne,1000,fichier) != NULL) // Tant que la ligne lue depuis le fichier n'est pas NULL faire ...
    {
...
String getLigne(FILE *fichier,int Index){
    ...
    for(int i=0;i<Index;i++){  // Tant que i est inférieure a l'Index
        fgets(Ligne,1000,fichier);   // Lire une ligne "dans le vide"
    }

    return fgets(Ligne,1000,fichier); // retourne la ligne voulue
}
...
String getColonne(String Ligne,int Colonne){
    ...
    int indexY; // ''
    String LigneTampon = (String)calloc(1000,sizeof(char));  // Création d'une ligne tampon pour pas modifier la ligne actuelle du fichier
    ...
    free(LigneTampon);
    // +1 pour l'exclusion
    return strbcpy(Ligne,indexX + 1,indexY);
}
...
String* getListeColonne(FILE *fichier,int Colonne){
    ...
    String* ListeColonne = (String*)calloc(1000,sizeof(String*));

    for(int i=0;i<nbLigne;i++){
        ListeColonne[i] = (String)calloc(1000,sizeof(char));
...
void addLigneAutoIndex(FILE *fichier){
    ...
    String Ligne = (String)calloc(1000,sizeof(char));
    ...
    for(int i=0;i<nbEntry-1;i++){ // Ajouter tout les valeur sauf pour l'index qui se mes automatiquement
        ...
        fgets(Buffer,1000,stdin); // Entrée de la valeur
 ...
 * 
 * Etant donnee son fonctionnement (fermeture du fichier pour suprimmer puis renomme un même fichier)
 * il faut IMPERTIVEMENT repointer le nom du pointeur qui pointer vers l'ancien fichier 
 * 
 ...
 * FILE *Temp = fopen("Temp.txt", "w+");
 * ...
 * fichier = removeOneIndex(fichier,10);
 * 
*/
FILE* removeOneIndex(FILE *fichier,int IndexLigne){
    ...
    int nbLigne = getNbLigne(fichier);
    String Ligne = (String)calloc(1000,sizeof(char));
    String BufferLigne = (String)calloc(1000,sizeof(char));
    ...
    for(int i=0;i<nbLigne-IndexLigne-1;i++){
        ...
        addLigneAutoIndexSpecificString(Temp,strbcpy(BufferLigne,Index+4,strlen(BufferLigne)));
    }
 ...
 * FILE *Temp = fopen("Temp.txt", "w+");
 * ...
 * fichier = modifieOneLigne(fichier,10,"johnny.d@netflix.com|N3tfl!x&Chil1|");
 * 
*/
...
/*
 * Affiche une liste de puis un fichier passée en entrée qui dois respectée le formatage
 * Prend en entrée un fichier cible, un element cibler ainsi que la colonne ciblée
*/
void DisplaySpecifiqueListe(FILE *fichier,String Element,int Colonne){
    String* ListeColonne = (String*)calloc(1000,sizeof(String)); // Création d'une liste qui vas acceuillir les info de la colonne choisie
        ListeColonne = getListeColonne(fichier,Colonne); // Affecte les valeur

    for(int i=0;i<getNbElementListe(ListeColonne);i++){ // Pour tout les élément de la liste ...
        if(strcmp(Element,ListeColonne[i]) == 0){ // Eviter les répetition d'un même élément 
            printf("%s : %s\n",getLigneXcolonneY(fichier,0,Colonne),getLigneXcolonneY(fichier,i,Colonne)); // Affichage des info 
            for(int j=0;j<getNbColonne(fichier);j++){ // Eviter les répétition des même élément 
                if(strcmp(getLigneXcolonneY(fichier,0,Colonne),getLigneXcolonneY(fichier,0,j)) != 0 && strcmp("Index",getLigneXcolonneY(fichier,0,j)) != 0){
                    printf("    %s : %s\n",getLigneXcolonneY(fichier,0,j),getLigneXcolonneY(fichier,i,j)); // Affichage des info 
                }
            }
            printf("\n");
        }
...
void DisplaySpecifiqueTable(FILE *fichier,String Element,int Colonne){
    ...
    String* ListeLigne = (String*)calloc(1000,sizeof(String));
    ...
    for(int i=0;i<getNbElementListe(ListeColonne);i++){
        if(strcmp(toUpperCase(Element),toUpperCase(ListeColonne[i])) == 0){
            addInListe(ListeLigne,getLigne(fichier,i));
        }
    }
    ...
    for(int i=0;i<NbColonne;i++){
        if(TabCarMax[i] < (int)strlen(getLigneXcolonneY(fichier,0,i))){ // On caste les res en int au lieux de size_t
            TabCarMax[i] = (int)strlen(getLigneXcolonneY(fichier,0,i)); // ''
        }
    }
        ...
        if(BoolCouleur == 0){
            for(int i=1;i<getNbLigne(fichier);i++){
                if(strcmp(StringBuffer,getLigneXcolonneY(fichier,i,Colonne)) == 0){
                    for(int j=0;j<NbColonne;j++){
                    printf_GRAY("|  ");
                    printf_BLUE(getLigneXcolonneY(fichier,i,j));
                    printf(" ");
            ...
            for(int i=1;i<getNbLigne(fichier);i++){
                if(strcmp(StringBuffer,getLigneXcolonneY(fichier,i,Colonne)) == 0){
                    for(int j=0;j<NbColonne;j++){
                    printf_GRAY("|  ");
                    printf_YELLOW(getLigneXcolonneY(fichier,i,j));
                    printf(" ");
...
void DisplayListe(FILE *fichier){
    ...
    for(int i=1;i<getNbLigne(fichier);i++){
        for(int j=1;j<getNbColonne(fichier);j++){
            printf("%s : %s\n",getLigneXcolonneY(fichier,0,j),getLigneXcolonneY(fichier,i,j));
        }
        printf("\n");
    }
...
void DisplayOrdonnedListe(FILE *fichier,int colonne){
    String* ListeDejaAfficher = (String*)calloc(1000,sizeof(String));
    String Buffer = (String)calloc(1000,sizeof(char));
    ...
    for(int i=1;i<getNbLigne(fichier);i++){
        ...
        if(BoolFound == 0){
            ...
            if(BoolCouleur == 0){
                printf_BLUE(getLigneXcolonneY(fichier,0,colonne));
                printf(" : ");
                printf_BLUE(Buffer);
                printf("\n");
                    ...
                    if(strcmp(Buffer,getLigneXcolonneY(fichier,j,colonne)) == 0){
                        for(int k=1;k<getNbColonne(fichier);k++){
                            if(strcmp(getLigneXcolonneY(fichier,0,colonne),getLigneXcolonneY(fichier,0,k)) != 0){
                                printf("    ");
                                printf_BLUE(getLigneXcolonneY(fichier,0,k));
                                printf(" : ");
                                printf_BLUE(getLigneXcolonneY(fichier,j,k));
                                printf("\n");
                            }
                        }
                        printf("\n");
                ...
                printf_YELLOW(getLigneXcolonneY(fichier,0,colonne));
                printf(" : ");
                printf_YELLOW(Buffer);
                printf("\n");
                for(int j=1;j<getNbLigne(fichier);j++){
                    if(strcmp(Buffer,getLigneXcolonneY(fichier,j,colonne)) == 0){
                        for(int k=1;k<getNbColonne(fichier);k++){
                            if(strcmp(getLigneXcolonneY(fichier,0,colonne),getLigneXcolonneY(fichier,0,k)) != 0){
                                printf("    ");
                                printf_YELLOW(getLigneXcolonneY(fichier,0,k));
                                printf(" : ");
                                printf_YELLOW(getLigneXcolonneY(fichier,j,k));
                                printf("\n");
                            }
                        }
                        printf("\n");
